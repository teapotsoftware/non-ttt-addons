...
function SWEP:Deploy()
	if SERVER then
		self:SetMaterial("!ToyGunMaterial")
	end
...
function SWEP:PrimaryAttack()
	...
	if self.Akimbo then
		local oddshot = (self:Clip1() % 2 == 0)
		self.Primary.Automatic = oddshot
		if oddshot then
	...
	if self.Primary.PitchOverride then
		self:EmitSound(self.Primary.Sound, 80, self.Primary.PitchOverride)
	else
...
function SWEP:GetOffset()
	if self:GetReloading() then return self.DefaultPos, self.DefaultAng end
	...
	return self.DefaultPos, self.DefaultAng
end
...
function SWEP:DrawWeaponSelection( x, y, w, h, a )
	...
	draw.SimpleText( self.IconLetter, self.SelectFont, x + w / 2, y + h * 0.65,
